'use client'
import React, { useState, useEffect } from 'react'
import {
  Thermometer,
  Baby,
  Moon,
  Sun,
  Snowflake,
  AlertTriangle,
  Check,
  Info,
  ChevronDown,
  Shield,
  Award,
  Heart,
  LeafyGreen,
  Flower,
  Layers,
  HelpCircle
} from 'lucide-react'

// TOG waarden voor kledinglagen - GESCHATTE WAARDEN (geen industrie-standaard)
const kledingWaarden = {
  'luier': { TOG: 0.1, naam: 'Alleen luier', info: 'Geschatte waarde' },
  'korte_romper': { TOG: 0.2, naam: 'Korte mouw romper', info: 'Geschatte waarde' },
  'lange_romper': { TOG: 0.4, naam: 'Lange mouw romper', info: 'Geschatte waarde' },
  'dun_slaappak': { TOG: 0.6, naam: 'Dun slaappakje', info: 'Geschatte waarde' },
  'dik_slaappak': { TOG: 0.9, naam: 'Dik slaappakje', info: 'Geschatte waarde' },
  'vestje': { TOG: 0.3, naam: 'Vestje', info: 'Geschatte waarde' },
  'sokjes': { TOG: 0.1, naam: 'Sokjes', info: 'Geschatte waarde' }
}

// Deken waarden - Met onderscheid tussen ingestopt en los
const dekenWaarden = {
  'ingestopt_laken': { TOG: 0.3, naam: 'Ingestopt lakentje', info: 'Veilig indien goed ingestopt' },
  'ingestopt_katoen': { TOG: 0.5, naam: 'Ingestopte katoenen deken', info: 'Stevig onder armen instoppen' },
  'ingestopt_muslin': { TOG: 0.8, naam: 'Ingestopte hydrofiele deken', info: 'Lichtgewicht, ademend' },
  'los_deken': { TOG: 1.5, naam: 'Losse deken (12+ mnd)', info: 'ALLEEN voor 12+ maanden!' }
}

// Functie om seizoen te bepalen op basis van huidige datum
const getHuidigSeizoen = () => {
  const maand = new Date().getMonth() + 1 // 1-12
  if (maand >= 3 && maand <= 5) return 'lente'
  if (maand >= 6 && maand <= 8) return 'zomer'
  if (maand >= 9 && maand <= 11) return 'herfst'
  return 'winter'
}

// Functie om geschatte buitentemperatuur te bepalen
const getGeschatteBuitenTemp = () => {
  const seizoen = getHuidigSeizoen()
  const timezone = Intl.DateTimeFormat().resolvedOptions().timeZone

  // Detecteer of gebruiker in Nederland/België is (CET timezone)
  const isNederlands = timezone.includes('Amsterdam') || timezone.includes('Brussels') || timezone.includes('Europe')

  if (!isNederlands) {
    // Voor andere locaties: algemene schatting op basis van seizoen
    const temps = { lente: 12, zomer: 22, herfst: 10, winter: 4 }
    return temps[seizoen]
  }

  // Voor Nederland: specifiekere schatting op basis van seizoen en maand
  const maand = new Date().getMonth() + 1

  if (seizoen === 'winter') {
    if (maand === 12) return 5
    if (maand === 1) return 3
    return 4 // februari
  }
  if (seizoen === 'lente') {
    if (maand === 3) return 8
    if (maand === 4) return 11
    return 15 // mei
  }
  if (seizoen === 'zomer') {
    if (maand === 6) return 18
    if (maand === 7) return 20
    return 19 // augustus
  }
  // herfst
  if (maand === 9) return 16
  if (maand === 10) return 12
  return 7 // november
}

export default function UltimateTOGCalculator() {
  // Bepaal seizoen en buitentemp dynamisch
  const huidigSeizoen = getHuidigSeizoen()
  const geschatteBuitenTemp = getGeschatteBuitenTemp()

  // State management
  const [kamerTemp, setKamerTemp] = useState(19)
  const [slaapzakTOG, setSlaapzakTOG] = useState(1.0)
  const [gekozenKleding, setGekozenKleding] = useState(['luier', 'lange_romper'])
  const [gekozenDeken, setGekozenDeken] = useState('')
  const [babyLeeftijd, setBabyLeeftijd] = useState('3-6')
  const [seizoen, setSeizoen] = useState(huidigSeizoen)
  const [buitenTemp, setBuitenTemp] = useState(geschatteBuitenTemp)
  const [isPremature, setIsPremature] = useState(false)
  const [showInfoModal, setShowInfoModal] = useState(false)
  const [safetyChecks, setSafetyChecks] = useState({
    neck: false,
    noSweat: false,
    chest: false,
    sleepQuality: false
  })

  // Clear loose blanket selection if age changes to under 12 months
  useEffect(() => {
    if (babyLeeftijd !== '12+' && gekozenDeken === 'los_deken') {
      setGekozenDeken('')
    }
  }, [babyLeeftijd, gekozenDeken])

  // VERBETERDE TOG aanbevelingen op basis van onderzoek
  const berekenAanbevolenTOG = (temp) => {
    if (temp >= 24) return { min: 0.2, ideal: 0.5, max: 0.5 }
    if (temp >= 22) return { min: 0.5, ideal: 0.8, max: 1.0 }
    if (temp >= 20) return { min: 1.0, ideal: 1.2, max: 2.5 }
    if (temp >= 18) return { min: 2.5, ideal: 2.5, max: 2.5 } // AANGEPAST van 1.8 naar 2.5
    if (temp >= 16) return { min: 2.5, ideal: 2.5, max: 3.5 }
    return { min: 3.5, ideal: 3.5, max: 3.5 }
  }

  // Bereken huidige TOG van kleding
  const berekenKledingTOG = () => {
    const kledingTOG = gekozenKleding.reduce((totaal, item) => {
      return totaal + (kledingWaarden[item]?.TOG || 0)
    }, 0)
    const dekenTOG = gekozenDeken ? (dekenWaarden[gekozenDeken]?.TOG || 0) : 0
    return kledingTOG + dekenTOG
  }

  const aanbevolenTOGRange = berekenAanbevolenTOG(kamerTemp)
  const kledingTOG = berekenKledingTOG()
  const totaleTOG = slaapzakTOG + kledingTOG
  
  // Bepaal status met verbeterde logica
  let status = 'perfect'
  let statusKleur = 'green'
  let statusTekst = 'Perfect! Ideale warmte voor je baby.'
  let statusIcon = <Check className="w-5 h-5" />
  
  if (totaleTOG < aanbevolenTOGRange.min) {
    const verschil = aanbevolenTOGRange.min - totaleTOG
    if (verschil > 0.5) {
      status = 'te_koud'
      statusKleur = 'red'
      statusIcon = <AlertTriangle className="w-5 h-5" />
      statusTekst = 'Te koud! Voeg warmere lagen toe voor comfort.'
    } else {
      status = 'iets_te_koud'
      statusKleur = 'orange'
      statusIcon = <Info className="w-5 h-5" />
      statusTekst = 'Iets te koud. Overweeg een extra laagje.'
    }
  } else if (totaleTOG > aanbevolenTOGRange.max) {
    const verschil = totaleTOG - aanbevolenTOGRange.max
    if (verschil > 0.5) {
      status = 'te_warm'
      statusKleur = 'red'
      statusIcon = <AlertTriangle className="w-5 h-5" />
      statusTekst = 'Te warm! Verwijder lagen om oververhitting te voorkomen.'
    } else {
      status = 'iets_te_warm'
      statusKleur = 'orange'
      statusIcon = <Info className="w-5 h-5" />
      statusTekst = 'Iets te warm. Overweeg een lichtere slaapzak of minder lagen.'
    }
  }

  // Get clothing recommendations based on temperature
  const getClothingRecommendations = () => {
    const is12Plus = babyLeeftijd === '12+'
    const isNewborn = babyLeeftijd === '0-3'
    
    if (kamerTemp < 16) {
      return [
        'Lange mouw romper', 
        'Warm slaappakje', 
        isNewborn ? '2.5-3.5 TOG slaapzak (50-60cm) of ingestopt dekentje' : '3.5 TOG slaapzak', 
        'Let op: kamer is kouder dan aanbevolen',
        is12Plus ? 'Evt. extra deken mogelijk' : 'Gebruik "feet to foot" methode bij dekens'
      ]
    } else if (kamerTemp < 18) {
      return [
        'Lange mouw romper', 
        'Katoenen slaappakje', 
        isNewborn ? '2.5 TOG slaapzak (vanaf 50cm) of ingestopte dekens' : '2.5-3.5 TOG slaapzak',
        is12Plus ? 'Deken mogelijk maar niet nodig' : 'Bij dekens: stevig instoppen!'
      ]
    } else if (kamerTemp < 20) {
      return [
        'Lange mouw romper of slaappakje', 
        isNewborn ? '1.0-2.5 TOG slaapzak of lichtgewicht ingestopte deken' : '2.5 TOG slaapzak (NHS aanbeveling)',
        is12Plus ? 'Geen deken nodig met goede slaapzak' : 'Slaapzak is veiliger dan dekens'
      ]
    } else if (kamerTemp < 22) {
      return [
        'Korte of lange mouw romper', 
        '1.0-2.5 TOG slaapzak afhankelijk van seizoen',
        isNewborn ? 'Of alleen ingestopt lakentje' : 'Geen deken nodig'
      ]
    } else if (kamerTemp < 24) {
      return [
        'Korte mouw romper', 
        '0.5-1.0 TOG slaapzak',
        'Of alleen ingestopt lakentje',
        'Zeker geen deken nodig'
      ]
    } else {
      return [
        'Alleen romper of luier', 
        '0.2-0.5 TOG slaapzak of alleen lakentje',
        'Te warm voor dekens!'
      ]
    }
  }

  const toggleKleding = (item) => {
    if (gekozenKleding.includes(item)) {
      setGekozenKleding(gekozenKleding.filter(k => k !== item))
    } else {
      setGekozenKleding([...gekozenKleding, item])
    }
  }

  const handleSafetyCheck = (check) => {
    setSafetyChecks(prev => ({ ...prev, [check]: !prev[check] }))
  }

  const allChecksPassed = Object.values(safetyChecks).every(check => check)

  // Get season icon
  const getSeasonIcon = () => {
    switch(seizoen) {
      case 'winter': return <Snowflake className="w-5 h-5 text-secondary" />
      case 'zomer': return <Sun className="w-5 h-5 text-primary" />
      case 'herfst': return <LeafyGreen className="w-5 h-5 text-primary/10" />
      case 'lente': return <Flower className="w-5 h-5 text-primary" />
      default: return <Sun className="w-5 h-5 text-secondary" />
    }
  }

  return (
    <div className="max-w-6xl mx-auto px-4 py-6 bg-white rounded-2xl">
        {/* Info Modal */}
        {showInfoModal && (
          <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-2xl p-6 max-w-2xl max-h-[80vh] overflow-y-auto">
              <h3 className="text-xl font-bold mb-4 text-text-primary">Over TOG-waarden</h3>
              
              <div className="space-y-4 text-sm text-text-secondary">
                <div className="p-4 rounded-lg bg-orange-50 border-l-4 border-orange-500">
                  <p className="font-semibold text-orange-800 mb-2">Belangrijke informatie:</p>
                  <p>De TOG-waarden voor individuele kledingstukken in deze calculator zijn <strong>schattingen</strong>. 
                  De industrie kent alleen officiële TOG-waarden toe aan slaapzakken en inbakerdoeken, niet aan rompers, luiers of sokjes.</p>
                </div>
                
                <div>
                  <h4 className="font-semibold text-text-primary mb-2">Wat is TOG?</h4>
                  <p>TOG staat voor "Thermal Overall Grade" - een maat voor thermische isolatie. 
                  Één TOG = 0.1 m²·K/W warmteweerstand.</p>
                </div>

                <div>
                  <h4 className="font-semibold text-text-primary mb-2">Officiële aanbevelingen:</h4>
                  <ul className="space-y-1">
                    <li>• <strong>24°C+:</strong> 0.2-0.5 TOG</li>
                    <li>• <strong>22-24°C:</strong> 0.5-1.0 TOG</li>
                    <li>• <strong>20-22°C:</strong> 1.0-2.5 TOG</li>
                    <li>• <strong>18-20°C:</strong> 2.5 TOG (NHS/Lullaby Trust)</li>
                    <li>• <strong>16-18°C:</strong> 2.5-3.5 TOG</li>
                    <li>• <strong>&lt;16°C:</strong> 3.5 TOG maximum</li>
                  </ul>
                </div>

                <div>
                  <h4 className="font-semibold text-text-primary mb-2">Dekens & Veiligheid:</h4>
                  <ul className="space-y-1">
                    <li>• <strong className="text-red-600">Losse dekens:</strong> NOOIT veilig voor baby's</li>
                    <li>• <strong className="text-green-600">Ingestopte dekens:</strong> Toegestaan mits correct gedaan</li>
                    <li>• <strong>"Feet to foot" methode:</strong> Voetjes aan voeteneind, deken onder armen</li>
                    <li>• <strong>Slaapzakken:</strong> Vanaf 50cm voor newborns beschikbaar</li>
                    <li>• <strong>12+ maanden:</strong> Losse dekens mogelijk maar niet ideaal</li>
                  </ul>
                </div>

                <div>
                  <h4 className="font-semibold text-text-primary mb-2">Veilig instoppen instructie:</h4>
                  <ol className="space-y-1 list-decimal list-inside text-sm">
                    <li>Leg baby met voetjes aan voeteneind bedje</li>
                    <li>Gebruik lichtgewicht deken(s)</li>
                    <li>Trek deken tot schouders/oksels (niet hoger!)</li>
                    <li>Stop deken stevig onder matras aan beide zijden</li>
                    <li>Controleer of deken niet omhoog kan schuiven</li>
                  </ol>
                </div>

                <div>
                  <h4 className="font-semibold text-text-primary mb-2">Bronnen:</h4>
                  <p>NHS (UK), The Lullaby Trust, American Academy of Pediatrics (AAP), Red Nose Australia</p>
                </div>

                <div className="p-4 rounded-lg bg-blue-50 border-l-4 border-blue-500">
                  <p className="font-semibold text-blue-800 mb-2">Advies:</p>
                  <p>Gebruik deze calculator als richtlijn, maar vertrouw vooral op je eigen observatie. 
                  Controleer regelmatig de nek en borst van je baby op tekenen van oververhitting of onderkoeling.</p>
                </div>
              </div>

              <button
                onClick={() => setShowInfoModal(false)}
                className="mt-6 w-full py-3 bg-primary text-white rounded-lg font-semibold"
              >
                Sluiten
              </button>
            </div>
          </div>
        )}

        <div className="grid lg:grid-cols-2 gap-6">
          {/* Left Column - Input Section */}
          <div className="space-y-6">
            {/* Weather & Room Info Card */}
            <div className="rounded-2xl p-6">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-lg font-semibold text-text-primary">
                  Kamer Informatie
                </h2>
                <span className="text-2xl font-bold text-primary">{kamerTemp}°C</span>
              </div>

              <div className="grid grid-cols-3 gap-4 pb-4 border-b border-border">
                <div className="text-center">
                  <Thermometer className="w-5 h-5 mx-auto mb-1 text-primary" />
                  <div className="text-xl font-bold">{kamerTemp}°C</div>
                  <div className="text-xs text-text-secondary">Binnen</div>
                </div>
                <div className="text-center border-x border-border">
                  <Thermometer className="w-5 h-5 mx-auto mb-1 text-secondary" />
                  <div className="text-xl font-bold">{buitenTemp}°C</div>
                  <div className="text-xs text-text-secondary">Buiten</div>
                </div>
                <div className="text-center">
                  <div className="mx-auto mb-1 flex justify-center">{getSeasonIcon()}</div>
                  <div className="text-xl font-bold capitalize">{seizoen}</div>
                  <div className="text-xs text-text-secondary">Seizoen</div>
                </div>
              </div>

              {/* Temperature Slider */}
              <div className="mt-6">
                <label className="block text-sm font-medium mb-3 text-text-primary">
                  Kamertemperatuur instellen
                </label>
                <input
                  type="range"
                  min="14"
                  max="28"
                  value={kamerTemp}
                  onChange={(e) => setKamerTemp(Number(e.target.value))}
                  className="w-full h-2 rounded-lg appearance-none cursor-pointer bg-gradient-to-r from-secondary via-primary to-accent"
                />
                <div className="flex justify-between mt-2 text-xs text-text-secondary">
                  <span className="flex items-center gap-1"><Snowflake className="w-3 h-3" /> 14°C</span>
                  <span className="font-bold text-primary">Ideaal: 18-22°C (AAP: 20-22°C)</span>
                  <span className="flex items-center gap-1"><Sun className="w-3 h-3" /> 28°C</span>
                </div>
              </div>

              {/* Temperature Warning - UPDATED */}
              {kamerTemp < 16 && (
                <div className="mt-4 p-3 rounded-lg flex items-start gap-2 bg-blue-50 border-l-4 border-blue-500">
                  <Snowflake className="w-4 h-4 mt-0.5 flex-shrink-0 text-blue-500" />
                  <p className="text-sm text-text-primary">Kamertemperatuur is lager dan aanbevolen (16-20°C). Gebruik 3.5 TOG slaapzak met warme kleding.</p>
                </div>
              )}
              {kamerTemp > 24 && (
                <div className="mt-4 p-3 rounded-lg flex items-start gap-2 bg-orange-50 border-l-4 border-orange-500">
                  <AlertTriangle className="w-4 h-4 mt-0.5 flex-shrink-0 text-orange-500" />
                  <p className="text-sm text-text-primary">Kamertemperatuur is hoog. Risico op oververhitting! Gebruik maximaal 0.5 TOG.</p>
                </div>
              )}
              {kamerTemp >= 16 && kamerTemp <= 24 && (
                <div className="mt-4 p-3 rounded-lg flex items-start gap-2 bg-green-50 border-l-4 border-green-500">
                  <Check className="w-4 h-4 mt-0.5 flex-shrink-0 text-green-500" />
                  <p className="text-sm text-text-primary">
                    {kamerTemp >= 20 && kamerTemp <= 22 
                      ? 'Ideale temperatuur volgens AAP (20-22°C)' 
                      : 'Acceptabele temperatuur voor veilige babyslaap'}
                  </p>
                </div>
              )}
            </div>

            {/* Baby Settings Card */}
            <div className="rounded-2xl p-6">
              <h2 className="text-lg font-semibold mb-4 font-poppins text-text-primary">
                <Baby className="inline-block w-5 h-5 mr-2 text-primary" />
                Baby Informatie
              </h2>

              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium mb-2 text-text-primary">
                    Leeftijd
                  </label>
                  <div className="relative">
                    <select
                      value={babyLeeftijd}
                      onChange={(e) => setBabyLeeftijd(e.target.value)}
                      className="w-full px-4 py-3 pr-10 rounded-lg border border-border appearance-none cursor-pointer focus:outline-none focus:ring-2 focus:ring-primary"
                    >
                      <option value="0-3">0-3 maanden</option>
                      <option value="3-6">3-6 maanden</option>
                      <option value="6-12">6-12 maanden</option>
                      <option value="12+">12+ maanden</option>
                    </select>
                    <ChevronDown className="absolute right-3 top-1/2 -translate-y-1/2 w-5 h-5 pointer-events-none text-text-secondary" />
                  </div>
                </div>

                <div>
                  <label className="block text-sm font-medium mb-2 text-text-primary">
                    Seizoen
                  </label>
                  <div className="relative">
                    <select
                      value={seizoen}
                      onChange={(e) => setSeizoen(e.target.value)}
                      className="w-full px-4 py-3 pr-10 rounded-lg border border-border appearance-none cursor-pointer focus:outline-none focus:ring-2 focus:ring-primary"
                    >
                      <option value="winter">Winter</option>
                      <option value="lente">Lente</option>
                      <option value="zomer">Zomer</option>
                      <option value="herfst">Herfst</option>
                    </select>
                    <ChevronDown className="absolute right-3 top-1/2 -translate-y-1/2 w-5 h-5 pointer-events-none text-text-secondary" />
                  </div>
                </div>
              </div>

              <div className="mt-4">
                <label className={`flex items-center p-3 rounded-lg cursor-pointer hover:bg-gray-50 ${isPremature ? 'bg-orange-50' : ''}`}>
                  <input
                    type="checkbox"
                    checked={isPremature}
                    onChange={(e) => setIsPremature(e.target.checked)}
                    className="w-5 h-5 rounded accent-primary"
                  />
                  <span className="ml-3 text-sm text-text-primary">
                    Prematuur geboren
                  </span>
                </label>
                {isPremature && (
                  <div className="mt-2 p-3 rounded-lg bg-orange-50 border-l-4 border-orange-500 flex items-start gap-2">
                    <AlertTriangle className="w-4 h-4 mt-0.5 flex-shrink-0 text-orange-600" />
                    <p className="text-sm text-orange-800">
                      Voor premature baby's wordt geadviseerd contact op te nemen met uw consultatiebureau voor specifiek advies.
                    </p>
                  </div>
                )}
              </div>
            </div>

            {/* Clothing Layers Card - UPDATED */}
            <div className="rounded-2xl p-6">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-lg font-semibold font-poppins text-text-primary">
                  <Layers className="inline-block w-5 h-5 mr-2 text-primary" />
                  Kledinglagen
                </h2>
                <button
                  onClick={() => setShowInfoModal(true)}
                  className="p-2 rounded-lg hover:bg-gray-100 transition-colors"
                  title="Info over TOG-waarden"
                >
                  <HelpCircle className="w-5 h-5 text-text-secondary" />
                </button>
              </div>

              {/* Info Alert */}
              <div className="mb-4 p-3 rounded-lg bg-blue-50 border-l-4 border-blue-500">
                <p className="text-xs text-blue-800">
                  <strong>Let op:</strong> TOG-waarden voor kleding zijn schattingen. Alleen slaapzakken hebben officiële TOG-ratings.
                </p>
              </div>

              {/* Slaapzak TOG Selection */}
              <div className="mb-6">
                <label className="block text-sm font-medium mb-3 text-text-primary">
                  Slaapzak TOG waarde (officiële rating)
                </label>
                <div className="grid grid-cols-5 gap-2">
                  {[0.2, 0.5, 1.0, 2.5, 3.5].map(tog => (
                    <button
                      key={tog}
                      onClick={() => setSlaapzakTOG(tog)}
                      className={`py-3 px-2 rounded-lg font-semibold text-sm transition-all border-2 ${
                        slaapzakTOG === tog ? 'bg-primary text-white border-primary' : 'bg-primary/10 text-gray-700 border-transparent'
                      }`}
                    >
                      {tog}
                    </button>
                  ))}
                </div>
              </div>

              {/* Clothing Items */}
              <label className="block text-sm font-medium mb-3 text-text-primary">
                Kleding onder slaapzak (geschatte waarden)
              </label>
              <div className="grid grid-cols-2 gap-3">
                {Object.entries(kledingWaarden).map(([item, info]) => {
                  const isSelected = gekozenKleding.includes(item)

                  return (
                    <button
                      key={item}
                      onClick={() => toggleKleding(item)}
                      className={`p-4 rounded-xl text-left transition-all transform hover:scale-[1.02] bg-background border-2 ${
                        isSelected ? 'border-primary' : 'border-transparent'
                      }`}
                    >
                      <div className="flex items-center justify-between">
                        <div className="flex-1">
                          <div className="font-medium text-sm text-text-primary">
                            {info.naam}
                          </div>
                          <div className="text-xs text-text-secondary">
                            ±{info.TOG} TOG
                          </div>
                        </div>
                        {isSelected && (
                          <Check className="w-5 h-5 flex-shrink-0 ml-2 text-primary" />
                        )}
                      </div>
                    </button>
                  )
                })}
              </div>

              {/* Dekens Sectie - NIEUW */}
              <div className="mt-6">
                <label className="block text-sm font-medium mb-3 text-text-primary">
                  Dekens & Lakens
                </label>
                
                {/* Veiligheidsinformatie op basis van leeftijd */}
                {(babyLeeftijd === '0-3' || babyLeeftijd === '3-6' || babyLeeftijd === '6-12') ? (
                  <>
                    <div className="p-4 rounded-lg bg-green-50 border-l-4 border-green-500 mb-3">
                      <div className="flex items-start gap-2">
                        <Check className="w-5 h-5 flex-shrink-0 text-green-600 mt-0.5" />
                        <div>
                          <p className="text-sm font-semibold text-green-800 mb-1">
                            Veilig instoppen is toegestaan voor baby's
                          </p>
                          <p className="text-xs text-green-700 mb-2">
                            Dekens mogen gebruikt worden mits ze STEVIG INGESTOPT zijn volgens de "feet to foot" methode.
                          </p>
                          <ul className="text-xs text-green-700 space-y-1">
                            <li>• Baby met voetjes aan voeteneind van bedje</li>
                            <li>• Deken stevig instoppen onder matras</li>
                            <li>• Deken NIET hoger dan schouders/oksels</li>
                            <li>• Alleen lichtgewicht dekens gebruiken</li>
                            <li>• Of gebruik een slaapzak (vanaf 50cm verkrijgbaar!)</li>
                          </ul>
                        </div>
                      </div>
                    </div>
                    
                    <div className="grid grid-cols-2 gap-3">
                      <button
                        onClick={() => setGekozenDeken('')}
                        className={`p-4 rounded-xl text-left transition-all transform hover:scale-[1.02] bg-background border-2 ${
                          !gekozenDeken ? 'border-primary' : 'border-transparent'
                        }`}
                      >
                        <div className="flex items-center justify-between">
                          <div className="flex-1">
                            <div className="font-medium text-sm text-text-primary">
                              Alleen slaapzak
                            </div>
                            <div className="text-xs text-text-secondary">
                              Veiligste optie
                            </div>
                          </div>
                          {!gekozenDeken && (
                            <Check className="w-5 h-5 flex-shrink-0 ml-2 text-primary" />
                          )}
                        </div>
                      </button>
                      
                      {Object.entries(dekenWaarden).filter(([key]) => !key.includes('los')).map(([item, info]) => {
                        const isSelected = gekozenDeken === item

                        return (
                          <button
                            key={item}
                            onClick={() => setGekozenDeken(isSelected ? '' : item)}
                            className={`p-4 rounded-xl text-left transition-all transform hover:scale-[1.02] bg-background border-2 ${
                              isSelected ? 'border-primary' : 'border-transparent'
                            }`}
                          >
                            <div className="flex items-center justify-between">
                              <div className="flex-1">
                                <div className="font-medium text-sm text-text-primary">
                                  {info.naam}
                                </div>
                                <div className="text-xs text-text-secondary">
                                  ±{info.TOG} TOG - {info.info}
                                </div>
                              </div>
                              {isSelected && (
                                <Check className="w-5 h-5 flex-shrink-0 ml-2 text-primary" />
                              )}
                            </div>
                          </button>
                        )
                      })}
                    </div>

                    {gekozenDeken && gekozenDeken.includes('ingestopt') && (
                      <div className="mt-3 p-3 rounded-lg bg-yellow-50 border-l-4 border-yellow-500">
                        <p className="text-xs text-yellow-800">
                          <strong>Let op:</strong> Deze deken MOET stevig ingestopt worden! 
                          Nooit los laten liggen. Overweeg een slaapzak voor gemak en veiligheid.
                        </p>
                      </div>
                    )}
                  </>
                ) : (
                  <>
                    <div className="mb-3 p-3 rounded-lg bg-blue-50 border-l-4 border-blue-500">
                      <p className="text-xs text-blue-800">
                        <strong>12+ maanden:</strong> Dekens kunnen nu veiliger gebruikt worden. 
                        Een slaapzak blijft de veiligste optie, maar losse dekens zijn mogelijk.
                      </p>
                    </div>
                    
                    <div className="grid grid-cols-2 gap-3">
                      <button
                        onClick={() => setGekozenDeken('')}
                        className={`p-4 rounded-xl text-left transition-all transform hover:scale-[1.02] bg-background border-2 ${
                          !gekozenDeken ? 'border-primary' : 'border-transparent'
                        }`}
                      >
                        <div className="flex items-center justify-between">
                          <div className="flex-1">
                            <div className="font-medium text-sm text-text-primary">
                              Geen deken
                            </div>
                            <div className="text-xs text-text-secondary">
                              Alleen slaapzak (veiligst)
                            </div>
                          </div>
                          {!gekozenDeken && (
                            <Check className="w-5 h-5 flex-shrink-0 ml-2 text-primary" />
                          )}
                        </div>
                      </button>
                      
                      {Object.entries(dekenWaarden).map(([item, info]) => {
                        const isSelected = gekozenDeken === item

                        return (
                          <button
                            key={item}
                            onClick={() => setGekozenDeken(isSelected ? '' : item)}
                            className={`p-4 rounded-xl text-left transition-all transform hover:scale-[1.02] bg-background border-2 ${
                              isSelected ? 'border-primary' : 'border-transparent'
                            }`}
                          >
                            <div className="flex items-center justify-between">
                              <div className="flex-1">
                                <div className="font-medium text-sm text-text-primary">
                                  {info.naam}
                                </div>
                                <div className="text-xs text-text-secondary">
                                  ±{info.TOG} TOG - {info.info}
                                </div>
                              </div>
                              {isSelected && (
                                <Check className="w-5 h-5 flex-shrink-0 ml-2 text-primary" />
                              )}
                            </div>
                          </button>
                        )
                      })}
                    </div>
                  </>
                )}
              </div>
            </div>
          </div>

          {/* Right Column - Results Section */}
          <div className="space-y-6">
            {/* TOG Analysis Card - UPDATED */}
            <div className="rounded-2xl p-6">
              <h2 className="text-lg font-semibold mb-4 font-poppins text-text-primary">
                <Info className="inline-block w-5 h-5 mr-2 text-primary" />
                TOG Analyse
              </h2>

              {/* Status Alert */}
              <div
                className={`p-4 rounded-xl mb-6 flex items-start gap-3 border-l-4 ${
                  statusKleur === 'green' ? 'bg-green-50 border-green-500' :
                  statusKleur === 'orange' ? 'bg-orange-50 border-orange-500' : 'bg-red-50 border-red-500'
                }`}
              >
                <div className={statusKleur === 'green' ? 'text-green-500' : statusKleur === 'orange' ? 'text-orange-500' : 'text-red-500'}>
                  {statusIcon}
                </div>
                <div className="flex-1">
                  <div className="font-semibold text-sm mb-1 text-text-primary">
                    {status === 'perfect' ? 'Perfect!' :
                     status.includes('iets') ? 'Let op' : 'Aanpassing nodig'}
                  </div>
                  <div className="text-sm text-text-secondary">
                    {statusTekst}
                  </div>
                </div>
              </div>

              {/* TOG Values Display - UPDATED */}
              <div className="space-y-4">
                <div className="p-4 rounded-lg bg-green-50">
                  <div className="flex justify-between items-center mb-2">
                    <span className="text-sm font-medium text-text-primary">Aanbevolen TOG bereik</span>
                    <span className="text-xl font-bold text-green-600">
                      {aanbevolenTOGRange.min === aanbevolenTOGRange.max 
                        ? aanbevolenTOGRange.ideal.toFixed(1)
                        : `${aanbevolenTOGRange.min.toFixed(1)} - ${aanbevolenTOGRange.max.toFixed(1)}`}
                    </span>
                  </div>
                  <div className="text-xs text-green-700">
                    Op basis van {kamerTemp}°C (NHS/Lullaby Trust richtlijnen)
                  </div>
                </div>

                <div className="flex justify-between items-center p-4 rounded-lg bg-background">
                  <span className="text-sm font-medium text-text-primary">Huidige TOG</span>
                  <span className="text-2xl font-bold text-primary">{totaleTOG.toFixed(1)}</span>
                </div>

                <div className="text-center p-3 rounded-lg bg-gray-100">
                  <div className="text-xs text-text-secondary">
                    Kleding: ±{gekozenKleding.reduce((t, i) => t + (kledingWaarden[i]?.TOG || 0), 0).toFixed(1)} 
                    {gekozenDeken && ` + Deken: ±${dekenWaarden[gekozenDeken]?.TOG || 0}`} 
                    + Slaapzak: {slaapzakTOG} = {totaleTOG.toFixed(1)} TOG
                  </div>
                </div>
              </div>

              {/* Visual Baby Layers */}
              <div className="mt-6 rounded-xl bg-background h-64 flex items-center justify-center">
                <div className="text-center">
                  <div className="relative w-[140px] h-[180px] mx-auto">
                    {/* Baby core - always visible */}
                    <div className="absolute inset-[60px_50px] bg-accent rounded-full transition-all duration-300"></div>

                    {/* Dynamically render layers based on selected clothing */}
                    {gekozenKleding.map((item, index) => {
                      const insetVertical = 60 - ((index + 1) * 15);
                      const insetHorizontal = 50 - ((index + 1) * 12);
                      const opacity = 0.3 + (index * 0.1);
                      return (
                        <div
                          key={item}
                          className="absolute rounded-full transition-all duration-300"
                          style={{
                            top: `${insetVertical}px`,
                            bottom: `${insetVertical}px`,
                            left: `${insetHorizontal}px`,
                            right: `${insetHorizontal}px`,
                            background: index % 2 === 0 ? `rgba(180, 196, 174, ${opacity})` : `rgba(232, 93, 66, ${opacity})`
                          }}
                        ></div>
                      );
                    })}

                    {/* Slaapzak - outer layer based on TOG value */}
                    <div
                      className="absolute inset-0 rounded-full transition-all duration-300"
                      style={{
                        background: slaapzakTOG >= 2.5 ? 'rgba(180, 196, 174, 0.5)' : slaapzakTOG >= 1.5 ? 'rgba(180, 196, 174, 0.4)' : 'rgba(180, 196, 174, 0.3)'
                      }}
                    ></div>

                    {/* Deken - outermost layer if selected */}
                    {gekozenDeken && (
                      <div
                        className="absolute -inset-2 rounded-full transition-all duration-300"
                        style={{
                          background: `rgba(139, 69, 19, ${0.2 + (dekenWaarden[gekozenDeken]?.TOG || 0) * 0.1})`,
                          border: '2px dashed rgba(139, 69, 19, 0.4)'
                        }}
                      ></div>
                    )}
                  </div>
                  <p className="text-xs text-text-secondary mt-4">
                    Totale TOG: <span className="font-bold text-primary">{totaleTOG.toFixed(1)}</span>
                  </p>
                  <p className="text-[11px] text-gray-400 mt-1">
                    {gekozenKleding.length} kledingla{gekozenKleding.length === 1 ? 'ag' : 'gen'}
                    {gekozenDeken && ` + ${dekenWaarden[gekozenDeken]?.naam}`}
                    + {slaapzakTOG} TOG slaapzak
                  </p>
                </div>
              </div>
            </div>

            {/* Clothing Recommendations Card - UPDATED */}
            <div className="rounded-2xl p-6">
              <h2 className="text-lg font-semibold mb-4 font-poppins text-text-primary flex items-center gap-2">
                <Layers className="w-5 h-5 text-primary" />
                Aanbevolen Kleding
              </h2>
              <ul className="space-y-2">
                {getClothingRecommendations().map((item, index) => (
                  <li key={index} className="flex items-start gap-2">
                    <Check className="w-4 h-4 text-green-500 mt-0.5 flex-shrink-0" />
                    <span className="text-sm text-text-primary">{item}</span>
                  </li>
                ))}
              </ul>
            </div>

            {/* Safety Checklist Card - UPDATED */}
            <div className="rounded-2xl p-6">
              <h2 className="text-lg font-semibold mb-4 font-poppins text-text-primary">
                <Shield className="inline-block w-5 h-5 mr-2 text-primary" />
                Veiligheidscontrole
              </h2>

              {/* Additional Safety Warning */}
              <div className="mb-4 p-3 rounded-lg bg-yellow-50 border-l-4 border-yellow-500">
                <p className="text-xs text-yellow-800">
                  <strong>Belangrijk:</strong> TOG-waarden zijn richtlijnen. Check altijd zelf of je baby het comfortabel heeft.
                  Baby's verschillen in warmteregulatie.
                </p>
              </div>

              <div className="space-y-3">
                {[
                  { key: 'neck', label: 'Nek voelt warm maar niet zweterig' },
                  { key: 'noSweat', label: 'Geen vochtig haar, rode wangen of snelle ademhaling' },
                  { key: 'chest', label: 'Borst/rug voelt aangenaam warm (niet heet of koud)' },
                  { key: 'sleepQuality', label: 'Baby slaapt rustig zonder rillen of onrust' }
                ].map((check) => (
                  <label key={check.key} className="flex items-center p-3 rounded-lg cursor-pointer hover:bg-gray-50">
                    <input
                      type="checkbox"
                      checked={safetyChecks[check.key]}
                      onChange={() => handleSafetyCheck(check.key)}
                      className="w-5 h-5 rounded accent-primary"
                    />
                    <span className="ml-3 text-sm flex-1 text-text-primary">
                      {check.label}
                    </span>
                  </label>
                ))}
              </div>

              {allChecksPassed && (
                <div className="mt-4 p-3 rounded-lg bg-green-50 border-l-4 border-green-500 flex items-center gap-2">
                  <Check className="w-4 h-4 text-green-500" />
                  <p className="text-sm font-medium text-green-700">
                    Alle veiligheidscontroles geslaagd!
                  </p>
                </div>
              )}

              {/* Extra Safety Tips */}
              <div className="mt-4 p-3 rounded-lg bg-blue-50">
                <p className="text-xs text-blue-800 font-semibold mb-1">Onthoud:</p>
                <ul className="text-xs text-blue-700 space-y-1">
                  <li>• Geen mutsjes binnen (baby verliest warmte via hoofd)</li>
                  <li>• Gezicht moet altijd onbedekt blijven</li>
                  <li>• <strong>Losse dekens: NOOIT!</strong> Ingestopte dekens: alleen met "feet to foot" methode</li>
                  <li>• Slaapzakken bestaan vanaf 50cm voor newborns</li>
                  <li>• Bij twijfel: één laag minder is veiliger dan teveel</li>
                  <li>• Gebruik nooit TOG 4.0 of hoger (verhoogd SIDS-risico)</li>
                  <li>• Slaapzak is altijd veiliger dan dekens</li>
                </ul>
              </div>
            </div>

          </div>
        </div>

      {/* Footer Disclaimer - UPDATED */}
      <div className="mt-8 p-6 rounded-2xl text-center bg-gray-50">
        <p className="text-sm text-text-secondary mb-2">
          <strong className="text-text-primary">Disclaimer:</strong> Deze calculator geeft algemene adviezen op basis van richtlijnen van NHS, Lullaby Trust, AAP en Red Nose Australia. 
          De TOG-waarden voor individuele kledingstukken en dekens zijn schattingen, niet industrie-standaarden.
        </p>
        <p className="text-sm text-text-secondary mb-2">
          <strong className="text-orange-600">Dekenveiligheid:</strong> LOSSE dekens zijn gevaarlijk voor baby's. 
          Indien dekens gebruikt worden, moeten ze volgens de "feet to foot" methode stevig ingestopt worden. 
          Slaapzakken (vanaf 50cm voor newborns) zijn altijd de veiligere keuze.
        </p>
        <p className="text-sm text-text-secondary">
          Raadpleeg bij twijfel altijd je kinderarts of consultatiebureau. Elke baby is uniek en heeft mogelijk andere behoeften.
          Vertrouw op je eigen observatie en pas aan waar nodig.
        </p>
      </div>
    </div>
  )
}
